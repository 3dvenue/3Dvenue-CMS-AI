...
JavaScriptに極力依存しない設計思想のもと、PageSpeed Insights全項目100点を目指すコンセプトで開発されています。
...
- GLB・MP3ファイルのアップロードに関する不具合を修正。
- オリジナルの音声プレイヤーを廃止し、ブラウザ標準の `<audio>` タグによる再生に変更。
- エディターの表示に関する不具合を修正。
- SQLiteデータベースの拡張子を `.db` に変更し、DB Browser for SQLiteで標準的に開けるように変更。
- その他、テキストや表示内容などを調整。
...
音声再生にはブラウザ標準の `<audio>` タグを使用しています。
...
アカウント・パスワードには2バイト文字が使えます。全角の空白も立派なパスワードになります。アカウントがメールアドレスである必要もなく、任意の文字列で構いません。
...
データベースファイルには標準的な `.db` 拡張子を使用しており、DB Browser for SQLiteなどのSQLite対応ツールで直接開くことができます。
