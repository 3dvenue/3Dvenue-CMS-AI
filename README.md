...
Designed to minimize JavaScript dependency, with the goal of achieving a perfect 100 score across every category in PageSpeed Insights.
...
- Replaced the original custom audio player with the browser-native `<audio>` element.
- Fixed display issues in the editor.
- Changed the SQLite database file extension to `.db` so it can be opened directly with DB Browser for SQLite.
...
Audio playback uses the browser-native `<audio>` element.
...
Accounts and passwords support double-byte (multi-byte) characters — even a full-width space is a valid password. The account doesn't need to be an email address either; any string works.
...
The database uses the standard `.db` file extension and can be opened directly with SQLite-compatible tools such as DB Browser for SQLite.
